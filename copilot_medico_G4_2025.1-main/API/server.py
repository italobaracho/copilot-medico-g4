...
# NOVO ENDPOINT: Receber dados extraídos da página
@app.route('/api/extracted-data', methods=['POST'])
def handle_extracted_data():
    """
    Recebe os dados extraídos das páginas (ex: prontuário do Amplimed)
    e envia para o Gemini como contexto clínico da consulta.
    Pode vir em formato 'extracted_data' (JSON dicionário) ou 'extracted_content' (JSON lista).
    """
    try:
        data = request.json
        if not data:
        ...
        if not extracted_data and not extracted_content:
            return jsonify({"status": "error", "message": "Nenhum dado extraído fornecido."}), 400

        new_patient_id_generated = None

        # Garante a existência do paciente
        if not patient_id:
            patient_id = generate_patient_id()
            new_patient_id_generated = patient_id
            ensure_patient_exists(patient_id, name="Desconhecido")
        ...
        # Garante a existência da consulta
        if not consultation_id:
            patient_consultations = get_patient_consultations(patient_id)
            if patient_consultations:
                consultation_id = patient_consultations[0]["id"]
            else:
                consultation_id = add_consultation_to_patient(patient_id, "Consulta Inicial (Extração)")
                if not consultation_id:
                    raise Exception("Falha ao criar consulta para o paciente.")
        ...
        formatted_parts = []
        if extracted_data:
            # Formato enviado pelo popup (App.tsx)
            editable_notes = extracted_data.get('editableNotes')
            if editable_notes:
                formatted_parts.append(f"Notas Editáveis:\n{editable_notes}")
            create_inputs = extracted_data.get('createInputs')
            if create_inputs:
                if isinstance(create_inputs, list):
                    formatted_parts.append("Campos do Prontuário:\n" + "\n".join([f"- {item}" for item in create_inputs]))
                else:
                    formatted_parts.append(f"Campos do Prontuário:\n{create_inputs}")
        elif extracted_content:
            # Formato enviado pelo botão flutuante (background.js)
            formatted_parts.append("Dados extraídos do prontuário:")
            for item in extracted_content:
                if isinstance(item, dict):
                    role = item.get('role', 'Dado')
                    text = item.get('text', '')
        ...
        # Anonimiza o texto extraído
        filtered_extracted_text = text_filter.remover_nomes(formatted_text)

        context_message = f"Os seguintes dados foram extraídos do prontuário eletrônico do paciente:\n\n{filtered_extracted_text}\n\nPor favor, analise-o e responda com sugestões ou um resumo clínico, conforme apropriado."

        # Adiciona ao histórico da consulta
        add_message_to_consultation_history(patient_id, consultation_id, "user", context_message)

        # Envia ao Gemini
        ai_response_text = gemini_connection.send_message(patient_id, consultation_id, context_message)
        ...
        response_data = {
            "status": "success",
            "message": "Dados extraídos recebidos e enviados para a IA.",
            "ai_response": ai_response_text,
            "consultation_id": consultation_id
        }
        if new_patient_id_generated:
        ...
        import traceback
        traceback.print_exc()
        return jsonify({"status": "error", "message": f"Erro interno ao processar dados extraídos: {e}"}), 500
